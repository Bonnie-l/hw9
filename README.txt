...
Extra credit
I did one extra credit, which is check if a word that the user enters is valid, click check word button, but there is a problem, the resource for the dictionary that professor gave us(from /usr/share/dict/words) I try to link it, but from the web console, it tells me fail to load resource: the server responded with status of 404 (). so the check word button doesn`t work.
